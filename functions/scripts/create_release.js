...
(async () => {
  try {
    ...
    try {
      ...
      console.error(
          "ERROR: gh CLI no está instalado. Por favor instálalo primero.");
      process.exit(1);
    }
    ...
    if (status) {
      console.error(
          "ERROR: Tienes cambios sin comitear. Limpia la rama primero.");
      process.exit(1);
    }
    ...
    const confirmed = await confirmAction(
        `Last release version was "${latestTag}",
         do you want to create "${releaseVersion}" [Y/n]: `);
    if (!confirmed) {
    ...
    runCommand(
        ...
        --title "Release - ${releaseVersion}" --fill`);
